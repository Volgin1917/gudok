...
- `python3 -m unittest discover -s tests` — 352 теста (~1 с). **Не** вызывается из run.sh: тесты гоняют вручную или CI.
...
- Страница планов `plans.html` генерируется из `plans.py` + markdown-документов репозитория:
...
- **Локальная особенность этой машины**: `TestCiPush::test_bash_syntax` падает здесь из-за нерабочего WSL (bash не монтирует диск) — это не баг кода; на Linux/CI все 352 теста зелёные. Локально этот единичный фейл игнорируй.
