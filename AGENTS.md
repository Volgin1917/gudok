...
- `python3 -m unittest discover -s tests` — 354 теста (~1 с). **Не** вызывается из run.sh: тесты гоняют вручную или CI.
...
- **Локальная особенность этой машины**: `TestCiPush::test_bash_syntax` падает здесь из-за нерабочего WSL (bash не монтирует диск) — это не баг кода; на Linux/CI все 354 теста зелёные. Локально этот единичный фейл игнорируй.
