...
<td class="n">{diff}</td></tr>"""
    ...
    checks = [
        ("Закон Бенфорда: распределение первой цифры", "wait"),
        ("Последние цифры процентов и голосов", "wait"),
        ("Доля «круглых» процентов в сводке", "wait"),
        ("Недействительные бюллетени × явка (корреляция)", "wait"),
        ("Диаграмма Шпилькина: явка × результат", "wait"),
...
территориям; мобилизация отделяется от базового профиля территории и по методу Шпилькина —
диаграмма «явка × результат» по участкам.</p>
</div>
...
серверной фазы карта МО по явке считается по протоколам и показывается с разницей к среднему.
Регрессионный разрез «город/село, доходы, занятость» — целевой контур (вне stdlib-ядра).</p>
</div>
...
<p class="ednote">{dem} Цифровые тесты (Бенфорд, последние цифры, целые проценты) и диаграмма Шпилькина —
шкала «на проверку», а не обвинение. На stdlib считаются описательные метрики и списки участков,
формальное p-значение — целевой контур (см. <a href="../methods.html#m40">М-40</a>).</p>
</div>
