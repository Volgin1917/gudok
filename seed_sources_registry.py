...
KNOWN = {
    ...
    # ── сайты ОМСУ (Госвеб/NetCat, без RSS — парсер списка новостей в collector.py) ──
    "web:Администрация Ульяновска": ("пресс-служба", "Ульяновск",
                                     "Администрация города Ульяновска (официальный сайт ОМСУ на Госвеб)",
                                     "подтверждён",
                                     "ulmeria.gosuslugi.ru — официальный сайт ОМСУ на платформе Госвеб "
                                     "(домен gosuslugi.ru, robots.txt открыт, sitemap указан). Полнее TG-канала "
                                     "@ulmeria (тот отключён): дата публикации до секунды, рубрики сайта, фото. "
                                     "Пагинация только JS — за запрос ~10 свежих материалов, истории вглубь нет. "
                                     "Подключён 15.09.2026"),
    ...
    # ── районные газеты (черновик d4 → разморожены решением редакции 15.09.2026) ──
    "rss:Карсунский вестник": ("редакция",   "Карсунский р-н", None, "уточнить",
                               "районная газета, пятница, тираж 3910 (каталог smi.ulgov.ru, сверить); "
                               "гл. ред. Е.А. Беляева; подключена 15.09.2026; VK club174957497"),
    "rss:Кумәк көч":          ("редакция",   "Старокулаткинский р-н", None, "уточнить",
                               "газета Старокулаткинского р-на (татароязычная в печати); лента сайта "
                               "преимущественно русскоязычная — первый источник оси «язык меньшинств»; "
                               "kumiakk85@mail.ru, р.п. Старая Кулатка, ул. Куйбышева 22"),
    "rss:Звезда (Новомалыклинский р-н)": ("редакция", "Новомалыклинский р-н", None, "уточнить",
                               "«Звезда. 73 регион», пятница, тираж 1775; гл. ред. О.А. Дергачёва (каталог); "
                               "единственный район с живым TG (@gazetazvezda73) и RSS одновременно"),
    "rss:Кузоватовские вести": ("редакция",  "Кузоватовский р-н",
                               "ОАУ «Редакция газеты \"Кузоватовские вести\"»", "уточнить",
                               "среда, тираж 2750; и.о. директора/гл. ред. С.И. Белов (каталог) — "
                               "сверить с В.М. Трофимовым (сайт); старый домен kuzvesti.ru — заглушка"),
    "tg:gazetazvezda73":      ("редакция",   "Новомалыклинский р-н", None, "уточнить",
                               "TG-канал газеты «Звезда»; tier 1 (зарегистрированное СМИ), не T3 — "
                               "в иерархии проекта T3 = авторские/анонимные каналы. Не публикует "
                               "с 16.03.2026 (13 сообщений в архиве) — резерв; живой источник района — RSS"),
}
...
VOICE_OF = {"tg:dimitrovgradonline": "Димитровград", "tg:dimitrovgradd": "Димитровград",
            ...
            "web:Администрация Ульяновска": "Ульяновск",
            # районные газеты (d4, разморожены 15.09.2026)
            "rss:Карсунский вестник": "Карсунский р-н",
            "rss:Кумәк көч": "Старокулаткинский р-н",
            "rss:Звезда (Новомалыклинский р-н)": "Новомалыклинский р-н",
            "rss:Кузоватовские вести": "Кузоватовский р-н",
            "tg:gazetazvezda73": "Новомалыклинский р-н"}
...
OWNER_FORM = {
    ...
    "web:Администрация Ульяновска": "официальные",
    ...
    # районные газеты — муниципальные/государственные учреждения; у Кузоватова юрлицо
    # названо на сайте, остальные — до верификации по ЕГРЮЛ (Волна 3)
    "rss:Кузоватовские вести": "государство",
}
...
# ── Канонические издания (15.09.2026, outlets.py) ─────────────────────────────
# Одна редакция = несколько каналов: RSS сайта, Telegram, сайт ОМСУ, ВКонтакте.
# Для метрик независимости (сеттеры повестки, каскады перепечаток, cluster_src,
# концентрация внимания) это ОДИН источник: перепечатка своего же материала
# в свой же канал не является независимым подтверждением.
# Объединяются только каналы ОДНОГО производителя (юрлицо/редакция/пресс-служба);
# конкурирующие каналы одного города остаются разными источниками.
# Основания — owner_verification.md, imprint сайтов, описания каналов в Telegram.
OUTLETS = {
    # ООО «Симбирск-Паблисити» (ИНН 7325027230): лента ulpressa.ru + TG редакции
    "tg:ulpressa": "Улпресса",
    "rss:Улпресса": "Улпресса",
    # ООО «Репортер 73» (ИНН 7325140155): телеканал, TG, сайт reporter73.tv
    "tg:reporter73": "Репортёр73",
    # районная газета «Звезда» (Новомалыклинский р-н): RSS + единственный живой TG района
    "tg:gazetazvezda73": "Звезда (Новомалыклинский р-н)",
    "rss:Звезда (Новомалыклинский р-н)": "Звезда (Новомалыклинский р-н)",
    # пресс-служба Администрации г. Ульяновска: официальный сайт ОМСУ + TG-канал
    # (решение редакции 15.09.2026: канал объявлен «неофициальным аккаунтом
    # пресс-службы», но ведёт его та же пресс-служба — один производитель).
    # Личный канал главы города @A_Boldakin остаётся отдельным источником.
    "web:Администрация Ульяновска": "Администрация г. Ульяновска",
    "tg:ulmeria": "Администрация г. Ульяновска",
    # исполнительная власть области: канал правительства + канал губернатора —
    # одна пресс-служба (решение редакции 15.09.2026; @ulgovru сам ссылается
    # на @Russkih_Aleksey как на канал губернатора, 11 дословных повторов за неделю)
    "tg:ulgovru": "Губернатор и Правительство Ульяновской области",
    "tg:Russkih_Aleksey": "Губернатор и Правительство Ульяновской области",
    # ПАО «УАЗ» (Соллерс): официальный TG-канал завода + корпоративный медиапортал
    "tg:UAZ_Today": "УАЗ (Соллерс)",
}

# Дополнительные написания одного и того же издания — встречаются в store.jsonl
# (подборки seed_data.py, домены, старые подписи). Ключ — id записи реестра.
AKA = {
    "tg:ulpressa": ["ulpressa", "ulpressa.ru", "t.me/ulpressa", "Улпресса"],
    "rss:Улпресса": ["Улпресса", "ulpressa.ru", "ulpressa"],
    "tg:reporter73": ["reporter73", "Репортёр73", "Репортер73", "Репортер 73",
                      "reporter73.tv", "t.me/reporter73"],
    "tg:gazetazvezda73": ["gazetazvezda73", "Звезда (Новомалыклинский р-н)",
                          "Газета «Звезда» Новомалыклинского района", "Звезда 73 регион"],
    "rss:Звезда (Новомалыклинский р-н)": ["Звезда (Новомалыклинский р-н)", "gazetazvezda73"],
    "web:Администрация Ульяновска": ["Администрация Ульяновска", "Администрация города Ульяновска",
                                     "ulmeria", "ulmeria.ru", "ulmeria.gosuslugi.ru"],
    "tg:ulmeria": ["ulmeria", "ulmeria.ru", "Администрация Ульяновска",
                   "Администрация города Ульяновска", "ulmeria.gosuslugi.ru"],
    "tg:ulgovru": ["ulgovru", "ulgov.gosuslugi.ru", "Правительство Ульяновской области",
                   "Ульяновская область", "t.me/ulgovru"],
    "tg:Russkih_Aleksey": ["Russkih_Aleksey", "Алексей Русских", "Алексей Русских (Telegram)",
                           "Русских Алексей Юрьевич", "t.me/Russkih_Aleksey"],
    "tg:UAZ_Today": ["UAZ_Today", "УАЗ Сегодня", "Sollers / УАЗ", "Соллерс / УАЗ",
                     "life.uaz.ru", "uaz.ru", "ПАО «УАЗ»"],
}
...
def main():
    ...
    for w in cfg.get("web_sources") or []:
        sid = f"web:{w['name']}"
        entries.append(_entry(sid, "web", w.get("title") or w["name"], w.get("tier"),
                              w.get("enabled", True), existing.get(sid)))
    ...
    registry = {
        "_meta": {
            ...
            # каналы одной редакции = одно издание (outlets.py): поле outlet + aka
            "outlets_note": "Поле outlet объединяет каналы одной редакции/пресс-службы в одно "
                            "издание: для метрик независимости (сеттеры повестки, каскады "
                            "перепечаток, cluster_src, концентрация внимания) RSS сайта и "
                            "Telegram той же редакции — ОДИН источник. Поле aka — написания "
                            "того же издания, встречающиеся в store.jsonl.",
            "outlets_total": len({e.get("outlet") or e["name"].lstrip("@") for e in entries}),
            "outlets_merged": sorted({e["outlet"] for e in entries if e.get("outlet")}),
        },
        "sources": entries,
    }
...
def _entry(sid, kind, name, tier, enabled, old):
    ...
    e = {
        ...
        # «аноним» — только для тех, у кого юрлица в принципе не заявлено (агрегаторы,
        # авторские и промо-каналы). Редакция и пресс-служба — зарегистрированные
        # организации: пока владелец не сверен по ЕГРЮЛ, это «не установлен», а не аноним
        "owner_form": (old or {}).get("owner_form") or OWNER_FORM.get(sid)
                      or ("не установлен" if prod in ("редакция", "пресс-служба")
                          else ("аноним" if not ((old or {}).get("owner") or owner)
                                else "не установлен")),
        "affiliate": (old or {}).get("affiliate") or AFFILIATE.get(sid),
        # каноническое издание: каналы одной редакции считаются одним источником
        # (outlets.py); aka — встречающиеся в store написания того же издания
        "outlet": (old or {}).get("outlet") or OUTLETS.get(sid),
        "aka": sorted(set([str(x) for x in ((old or {}).get("aka") or [])]
                          + [str(x) for x in AKA.get(sid, [])])),
